...
| C5 | **Reward claimed (reward.claim)** | Celebration screen — voucher auto-saved to "ของขวัญ" tab for next visit. New blank card starts immediately. The dock's ของขวัญ count bumps live via SSE (`gifts-update` on `/sse/me`) so other open tabs reflect the new voucher without a refresh. The threshold-th stamp triggers this page directly (no manual "เปิดของขวัญ" tap needed) — `shop.reward_threshold` drives the cutoff, so a shop set to 5 redeems on stamp #5. Auto-redeem lives inside `issue_point()`, so every stamping path (customer scan, shop-side scan, phone entry, manual insert, DeeReach `bonus_stamp_count`) gets it for free — no chance of an issuance route forgetting to fire. |
