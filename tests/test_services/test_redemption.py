...
from app.services.issuance import void_stamp
...
async def _seed_stamps(db, shop, customer, count: int) -> list[Stamp]:
...
async def test_active_count_initially_zero(db, shop, customer):
    assert await active_stamp_count(db, shop.id, customer.id) == 0


async def test_active_count_ignores_voided(db, shop, customer):
    stamps = await _seed_stamps(db, shop, customer, 3)
    await void_stamp(db, stamps[0])
    assert await active_stamp_count(db, shop.id, customer.id) == 2
...
async def test_void_redemption_restores_stamps(db, shop, customer):
    ...
    assert await active_stamp_count(db, shop.id, customer.id) == 10
