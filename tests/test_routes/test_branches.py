...
async def test_card_renders_branch_in_subtitle(named_client, db, shop):
    ...
    await named_client.get(f"/scan/{shop.id}?branch={branch.id}", follow_redirects=False)
    response = await named_client.get(f"/card/{shop.id}?branch={branch.id}")
    assert response.status_code == 200
    assert "ทองหล่อ" in response.text


async def test_card_remembers_last_branch_without_query(named_client, db, shop):
