...
def make_oauth_state(role: str = "shop") -> tuple[str, str]:
    """Returns (url_state, cookie_token).

    url_state goes into the LINE authorize URL.
    cookie_token is a signed JWT containing the nonce + role — stored in an
    httpOnly cookie. On callback we verify the cookie token AND that its nonce
    matches the URL state, and read the role to decide whether to log the
    caller in as a Shop ("shop") or a Customer ("customer").
    """
...
def verify_oauth_state(url_state: str, cookie_token: Optional[str]) -> Optional[str]:
    """Returns the role ("shop" / "customer") on success, None on failure.

    The caller treats None as "reject the callback"; otherwise branches on
    the returned role to pick the right login path.
    """
    if not cookie_token:
        return None
    try:
    ...
    if payload.get("nonce") != url_state:
        return None
    return payload.get("role") or "shop"
